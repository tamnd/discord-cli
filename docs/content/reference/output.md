...
describes what users get. Pick a format with `-o`, or let discord choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
discord <command> -o table   # aligned columns for reading
discord <command> -o jsonl   # one JSON object per line, for piping
discord <command> -o json    # a single JSON array
discord <command> -o csv     # spreadsheet friendly
discord <command> -o tsv     # tab-separated
discord <command> -o url     # just the URL column
discord <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
discord <command> --fields id,title,url
```
...
```bash
discord <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
discord <command>            # a table, because this is a terminal
discord <command> | wc -l    # JSONL, because this is a pipe
```
